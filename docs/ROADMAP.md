...
Phase 6  🔶  協作與部署         — 認證✅、多使用者隔離✅、Docker、CI/CD
```
...
**設計決策**：Metadata-based 過濾（非 per-user collection）— 避免 collection 爆炸，共享文件自然支援。

**隔離機制**：
- document metadata 加入 `user_id`（string）+ `shared`（"true"/"false"）
- 查詢時用 `where={"$or": [{"user_id": str(user_id)}, {"shared": "true"}]}`
- 自動索引文件 → `user_id="-1"`, `shared="true"`（所有使用者可見）
- 手動上傳文件 → `user_id=str(uid)`, `shared="false"`（僅上傳者可見）
- `user_id=None` → 不加 where（向後相容 dev 模式）

**user_id 穿透路徑**：
```
Router (auth token > request body)
  → Service (chat/email/report/form_filler)
    → sse_pipeline.rag_sse_stream()
      → sse_pipeline.search_all_collections()
        → document_service.search_documents(user_id=...)
...
```
修改/新增檔案：
├── app/services/document_service.py     # 核心：metadata + search 過濾
├── app/services/indexing_service.py      # 自動索引 shared 標記
├── app/services/sse_pipeline.py          # 穿透 user_id
├── app/services/rag_pipeline.py          # 穿透 user_id
├── app/services/form_filler.py           # 穿透 user_id（1 行）
├── app/services/chat_service.py          # 穿透 user_id
├── app/services/email_generator.py       # 穿透 user_id
├── app/services/report_generator.py      # 穿透 user_id
├── app/schemas/chat.py                   # +user_id 欄位
├── app/schemas/email.py                  # +user_id 欄位
├── app/schemas/report.py                 # +user_id 欄位
├── app/routers/chat.py                   # +auth 依賴 + user_id
├── app/routers/email.py                  # +auth 依賴 + user_id
├── app/routers/report.py                 # +auth 依賴 + user_id
├── app/routers/documents.py              # +auth + user_id
├── app/routers/indexing.py               # +auth
├── frontend/src/api/client.ts            # +sseHeaders + getCurrentUserId
├── frontend/src/api/chat.ts              # +auth + user_id
├── frontend/src/api/email.ts             # +auth + user_id
├── frontend/src/api/report.ts            # +auth + user_id
├── frontend/src/api/documents.ts         # +user_id
├── frontend/src/types/chat.ts            # +user_id 型別
├── frontend/src/types/email.ts           # +user_id 型別
├── frontend/src/types/report.ts          # +user_id 型別
├── scripts/migrate_chroma_metadata.py    # 新增：既有文件 migration
└── tests/test_multi_user_isolation.py    # 新增：27 tests
```
