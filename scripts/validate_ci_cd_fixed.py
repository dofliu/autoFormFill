    ...
    try:
        ...
        for key in required_keys:
            if key not in deploy_config:
                print(f"[ERROR] CD 工作流缺少必需鍵: {key}")
                return False
        
        # 檢查觸發條件
        triggers = deploy_config.get('on', {})
        if not triggers:
            print("[ERROR] CD 工作流缺少觸發條件")
            return False
...
def check_deployment_files():
    ...
    for filename, description in required_files:
        ...
        if not file_path.exists():
            print(f"[ERROR] {description}: {file_path} (文件不存在)")
            all_exist = False
        else:
            # 檢查文件是否可執行
            if os.access(file_path, os.X_OK):
                print(f"[OK] {description}: {file_path} (可執行)")
            else:
                print(f"[WARN] {description}: {file_path} (不可執行，請運行 chmod +x)")
...
def check_env_files():
    ...
    for filename, description in required_files:
        if not Path(filename).exists():
            print(f"[ERROR] {description}: {filename} (文件不存在)")
            all_exist = False
        else:
