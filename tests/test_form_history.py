"""
Tests for Form History feature — JobStore history methods (async, in-memory fallback)
"""
import pytest
...
class TestJobStoreHistory:
    """Test job store history functionality (in-memory mode)"""
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_by_user_empty(self):
        """Test getting jobs for a user with no jobs"""
        jobs = await self.store.get_jobs_by_user(999)
        assert jobs == []

    @pytest.mark.asyncio
    async def test_get_jobs_by_user_single(self):
        ...
        jobs = await self.store.get_jobs_by_user(1)
        assert len(jobs) == 1
        assert jobs[0]["job_id"] == job_id

    @pytest.mark.asyncio
    async def test_get_jobs_by_user_multiple(self):
        """Test getting jobs for a user with multiple jobs"""
        # Create jobs for user 1
        await self.store.create_job({"user_id": 1, "filename": "a.docx", "created_at": "2024-01-01T10:00:00"})
        await self.store.create_job({"user_id": 1, "filename": "b.docx", "created_at": "2024-01-03T10:00:00"})
        await self.store.create_job({"user_id": 1, "filename": "c.docx", "created_at": "2024-01-02T10:00:00"})
        # Create job for different user
        await self.store.create_job({"user_id": 2, "filename": "other.docx"})
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_by_user_limit(self):
        ...
        # Request limit of 10
        jobs = await self.store.get_jobs_by_user(1, limit=10)

        assert len(jobs) == 10

    @pytest.mark.asyncio
    async def test_get_jobs_by_template(self):
        ...
        jobs = await self.store.get_jobs_by_template("scholarship.docx", user_id=1)
    ...
    @pytest.mark.asyncio
    async def test_get_jobs_by_template_empty(self):
        """Test getting jobs by non-existent template"""
        jobs = await self.store.get_jobs_by_template("nonexistent.docx", user_id=1)
        assert jobs == []
